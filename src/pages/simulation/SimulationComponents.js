...
const SimulationForm = () => {
    ...
    const RenderSimulation = (props) => {
        ...
        const getActive = async (index) => {
            let response = await fetch('http://localhost:5000/get_active', {
                method: 'POST', 
                mode: 'cors', 
                cache: 'no-cache', 
                ...
                body: JSON.stringify({
                    'active' : simulations[index].existing_active
                })
            })
            let data = await response.text();
            var json = JSON.parse(data);
            let temp_simulation = simulations
            temp_simulation[index].existing_active = json.active
